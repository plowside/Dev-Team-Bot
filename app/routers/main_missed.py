...
from utils.functions import del_message
...
async def main_dialogue(message: Message, bot: Bot, state: FSMContext):
	...
	if message.content_type == 'text':
		...
		await bot.copy_message(chat_id=admin.tg_user_id, from_chat_id=user.tg_user_id, message_id=message.message_id)
	await DialogueMessagex.add(dialogue_id=dialogue.id, message_id=message.message_id, message_content_type=message.content_type, message_text=message.text, from_user_id=user.id, from_user_tg_id=user.tg_user_id, from_user_type='user')